<template>
      ...
      <v-col cols="12" sm="9" md="6">
        <v-card>
          <v-toolbar color="third">
            <v-toolbar-title>Reset Password Request</v-toolbar-title>
            
          </v-toolbar>
          <div>Please submit the email, so we will send you two emails for you to reset the password.  One mail has the link 
            to the page to reset.  The other mail will have the PIN needed to submit the reset form.
            </div>
          <v-card-text>
  ...
  function formSubmit(e: any) {
    ...
    isDone.value = false;
    axios
      .post(import.meta.env.VITE_API_URL + "/s1/main/reset-password-email", {
        email: email.value
      })
      .then((response) => {
        ...
        if (response.data.statusCode == "200") {
          ...
          if (response.data.statusCode == 10060) {
            msg.value = "Required field is missing (10060).";
          } else if (response.data.statusCode == 10061) {
